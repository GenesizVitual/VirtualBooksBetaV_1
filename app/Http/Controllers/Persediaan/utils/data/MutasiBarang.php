...
class MutasiBarang
{
    ...
    private static function daftar_mutasi_barang($data_barang)
    {
        ...
        $column['bukti'] = 'BAST';
        $column['masuk'] = number_format(self::$stok_penerimaan, 0,'','');
        $column['keluar'] = 0;
        $column['sisa'] = number_format(self::$stok, 0,'','');
        self::$stok += self::$stok_penerimaan;
        $column['sisa_pp'] = number_format(self::$stok, 0,'','');
        $column['satuan'] = $data_barang->satuan;
        $column['harga_beli'] = number_format(FormulaPajak::formula_pajak($data_barang->harga_barang, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
//        $column['total_penerimaan'] = number_format(FormulaPajak::formula_pajak($data_barang->harga_barang * $data_barang->jumlah_barang, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
        $column['total_penerimaan'] = number_format(FormulaPajak::formula_pajak($data_barang->harga_barang * self::$stok_penerimaan, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
        $column['total_pengeluaran'] = number_format(0, 0,'','');
        $column['id_barang'] = $data_barang->id_gudang;
        $column['total'] = number_format(FormulaPajak::formula_pajak(self::$total, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
        self::$total += ($data_barang->harga_barang * self::$stok_penerimaan);
        $column['total_akhir'] = number_format(FormulaPajak::formula_pajak(self::$total, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
        self::$row[] = $column;
        ...
        foreach ($data_barang->linkToDistribusi as $data_barang_keluar) {
            ...
            $column['nm_barang'] = $data_barang->linkToGudang->nama_barang;
            $column['masuk'] = 0;
            $column['keluar'] = number_format($data_barang_keluar->jumlah_keluar, 0,'','');
            self::$stok -= $data_barang_keluar->jumlah_keluar;
            $column['satuan'] = $data_barang->satuan;
            $column['harga_beli'] = number_format(FormulaPajak::formula_pajak($data_barang->harga_barang, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
            $column['total_penerimaan'] = number_format(0, 0,'','');
            $column['total_pengeluaran'] = number_format(FormulaPajak::formula_pajak($data_barang->harga_barang * $data_barang_keluar->jumlah_keluar, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
            $column['sisa_pp'] = number_format(self::$stok, 0,'','');
            $column['total'] = number_format(FormulaPajak::formula_pajak(self::$total, $data_barang->linkToNota->ppn, $data_barang->linkToNota->pph), 0,'','');
